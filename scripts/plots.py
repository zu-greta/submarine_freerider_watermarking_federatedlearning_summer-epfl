...
def savings_table(a):
    ...
    runs_all = load(a.inp)
    rows = []
    et_default, el_default = 0.064, 0.264
    for f in fams:
        runs = pick(runs_all, f) if f else runs_all
        if not runs:
            continue
        # thresholds: CLI overrides, else the run's own frozen/oracle etas, else defaults
        eta_t = a.eta_tight if a.eta_tight is not None else cfg(runs[0], "wm_eta_fixed", et_default) or et_default
        eta_l = a.eta_loose if a.eta_loose is not None else cfg(runs[0], "autop_oracle_eta", el_default) or el_default

        fr_cids, tclass, data, _twin = _submarine(runs)
        all_cids = sorted({int(k) for r in runs
                           for k in ((r.get("compute", {}) or {}).get("per_client", {}) or {})})
        honest = [c for c in all_cids if c not in set(fr_cids)]
        if not fr_cids or not honest:
            continue

        # honest cumulative samples (mean over honest clients and seeds)
        hon_tot = []
        for r in runs:
            for cid in honest:
                cb = _cumulative_by_round(r, cid, "samples")
                if cb:
                    hon_tot.append(max(cb.values()))
        hon_mean = float(np.mean(hon_tot)) if hon_tot else float("nan")
        ...
        for r in runs:
            ...
            er = s.get("effort_ratio_gpu")
            if er is not None:
                gsav.append(1.0 - float(er))
        gpu_saved = float(np.mean(gsav)) if gsav else float("nan")

        for cid in fr_cids:
            seeds = data[cid]["seeds"]
            W = data[cid]["defect"] or cfg(runs[0], "autop_honest_until", 12)
            # cumulative samples for this FR (mean over seeds)
            fr_tot = [max(s["samp"].values()) for s in seeds if s["samp"]]
            fr_mean = float(np.mean(fr_tot)) if fr_tot else float("nan")
            saved = 1 - fr_mean / hon_mean if hon_mean and hon_mean == hon_mean else float("nan")
            tapf = _tap_fraction(seeds)
            # post-defection server-measured BER, pooled over seeds and rounds >= W
            post = [b for s in seeds for rd, b in s["srv"].items() if rd >= W]
            ber_mu = float(np.mean(post)) if post else float("nan")
            ber_sd = float(np.std(post)) if post else float("nan")
            rec_t = (np.mean([b >= eta_t for b in post]) if post else float("nan"))
            rec_l = (np.mean([b >= eta_l for b in post]) if post else float("nan"))
            rows.append({
                "fam": (f or "runs").split("_rep")[0], "cid": cid, "cls": tclass.get(cid),
                "W": W, "saved": saved, "gpu_saved": gpu_saved, "tapf": tapf,
                "ber_mu": ber_mu, "ber_sd": ber_sd,
                "rec_t": rec_t, "rec_l": rec_l, "eta_t": eta_t, "eta_l": eta_l,
            })
    if not rows:
        print("savings_table: no rows (check --in glob / --families)"); return

    eta_t = rows[0]["eta_t"]; eta_l = rows[0]["eta_l"]
    esc = lambda s: str(s).replace("_", r"\_")
    L = []
    L.append("% Auto-generated by plots.py savings_table.")
    L.append(r"\begin{table}[t]")
    L.append(r"  \centering")
    L.append(r"  \caption{Free-rider cost. \textbf{Smp}=samples saved "
             r"$=1-$(FR samples)/(honest samples), device-independent and per free-rider; "
             r"\textbf{GPU}=GPU-time saved $=1-$(FR/honest GPU-ms) measured within the same run "
             r"(a ratio, not wall-clock; run-level, shared by both free-riders); "
             r"\textbf{Tap}=fraction of free-ride rounds the attacker trains (100\%=never coasts); "
             r"\textbf{BER}=server-measured bit-error rate after defection "
             f"(mean$\\pm$std over seeds$\\times$rounds); \\textbf{{Recall}}=fraction of free-ride "
             f"rounds flagged at each threshold ($\\eta_{{\\mathrm{{tight}}}}={eta_t:.3f}$, "
             f"$\\eta_{{\\mathrm{{loose}}}}={eta_l:.3f}$). Higher saved and lower recall favour "
             r"the attacker.}")
    L.append(r"  \label{tab:savings}")
    L.append(r"  \begin{tabular}{llcccccc}")
    L.append(r"    \toprule")
    L.append(r"    Setting & FR (class) & $W$ & Smp & GPU & Tap & BER (post) "
             r"& Recall $\eta_{\mathrm{t}}$/$\eta_{\mathrm{l}}$ \\")
    L.append(r"    \midrule")
    for r in rows:
        L.append("    {fam} & cid{cid} (c{cls}) & {W} & {saved}\\% & {gpu}\\% & {tapf}\\% "
                 "& ${bmu}\\pm{bsd}$ & {rt}\\%/{rl}\\% \\\\".format(
                     fam=esc(r["fam"]), cid=r["cid"], cls=r["cls"], W=r["W"],
                     saved=_fmt(r["saved"], 0, pct=True), gpu=_fmt(r["gpu_saved"], 0, pct=True),
                     tapf=_fmt(r["tapf"], 0, pct=True),
                     bmu=_fmt(r["ber_mu"], 3), bsd=_fmt(r["ber_sd"], 3),
                     rt=_fmt(r["rec_t"], 0, pct=True), rl=_fmt(r["rec_l"], 0, pct=True)))
    L.append(r"    \bottomrule")
    L.append(r"  \end{tabular}")
    L.append(r"\end{table}")
    tex = "\n".join(L) + "\n"

    out = a.out or "savings_table"
    ...
    for r in rows:                       # console echo for a quick sanity check
        print(f"  {r['fam']} cid{r['cid']}(c{r['cls']}): "
              f"smp {_fmt(r['saved'],0,pct=True)}%  gpu {_fmt(r['gpu_saved'],0,pct=True)}%  "
              f"tap {_fmt(r['tapf'],0,pct=True)}%  BER {_fmt(r['ber_mu'],3)}  "
              f"recall {_fmt(r['rec_t'],0,pct=True)}/{_fmt(r['rec_l'],0,pct=True)}%")
